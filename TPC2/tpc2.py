...
def main():
    if len(sys.argv) != 2:
        print("Usage: python3 modified_converter.py <\"MarkdownText\"/MarkdownFile.md>")
        sys.exit(1)
    ...
    if is_file(sys.argv[1]):
        ...
        output_file_name = "modifiedOutput.html"

        with open(output_file_name, 'w') as f:
            f.write(html_text)
        print(f"Modified HTML file created successfully: {output_file_name}")
    else:
